...
def write_module(by_assembly):
    with open(MODULE, "w", newline="\n") as f:
        f.write('"""HumCFS common fragile sites for hg19 and hg38.\n\n')
        f.write("Source: HumCFS (https://webs.iiitd.edu.in/raghava/humcfs/), GRCh38 per-chromosome\n")
        f.write("BED files. The hg38 set is the cleaned native HumCFS data; the hg19 set is lifted\n")
        f.write("over via the UCSC hg38->hg19 chain. Generated by scripts/build_fragile_sites.py.\n")
        f.write("Tuples are (chrom, start, end, name), 0-based half-open, matching the cytobands\n")
        f.write('module.\n"""\n')
...
def main():
    ...
    write_module({"hg19": hg19_sites, "hg38": hg38_sites})
